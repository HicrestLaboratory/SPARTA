...
//multiplies a VBS matrix (vbmatA) and dense matrix (B); stores B*A into (C)
    ...
    intT rows_in_block;
    intT* jab_loc = vbmatA.jab;
    std::vector<intT> h_d_A;
    std::vector<intT> h_d_B;
    std::vector<intT> h_d_C;
...
void cublas_blockmat_batchedBA_v2(const VBR& vbmatA, DataT* B, int B_rows, DataT_C* C, float& dt)
{
//multiplies a VBS matrix (vbmatA) and dense matrix (B); stores B*A into (C)
    //vbmatA:       column-major entries (in-block) storage;
    //              row-major block storage; 
