...
def best_barplot(dfs_reordering, square_matrices, rectangular_matrices, methods, fumbles = True, fumbles_parameter = "speedup_spmmcsr", make_legend = True, figsize = (5,4), min_best = True, parameter = "time_spmmcsr", ylabel = "", save_path = ""):
    fumbles_colors = ["#D3D3D3"]
    ...
    handles = [Patch(facecolor='white', edgecolor='grey', label=f'Square Matrices'),
                    Patch(facecolor='white', edgecolor='grey', label=f'Rectangular Matrices', hatch = "//"),
...
def best_barplot_bsize(dfs_reordering, square_matrices, rectangular_matrices, methods, fumbles = True, block_sizes = [32,64], fumbles_parameter = "speedup_spmmcsr", min_best = True, parameter = "time_spmmcsr", ylabel = "", save_path = ""):
        
        # Initialize data structures to collect counts
    square_counts = {b: {} for b in block_sizes}
    rect_counts = {b: {} for b in block_sizes}
    worse_counts = {b: {} for b in block_sizes}


    fumbles_color = "#D3D3D3"
    fumbles_color_palette = ["#D3D3D3", "#A9A9A9", "#808080", "#696969", "#505050"]

    b_size_hatches = ["","X",".."]

    # Loop over block sizes and collect counts
    for i, b in enumerate(block_sizes):
        # Update parameters for the current block size
        fumbles_parameter_b = f"{fumbles_parameter}_{b}"
        parameter_b = f"{parameter}_{b}"

        # Collect counts for each method
        square_counts[b] = count_best_method(dfs_reordering, square_matrices, parameter_b)
        rect_counts[b] = count_best_method(dfs_reordering, rectangular_matrices, parameter_b)

        # Collect fumbles counts
        worse_counts[b] = {}
        for method in methods:
            temp_method_df = dfs_reordering[method].copy()
            temp_method_df = set_allowed_matrices(temp_method_df, square_matrices)
            temp_method_df = find_best(temp_method_df, best_parameter=parameter_b, min=min_best)
            fumble_count = (temp_method_df[fumbles_parameter_b] < .98).sum()
            worse_counts[b][method] = -fumble_count  # Negative for plotting below x-axis

    # Prepare for plotting
    methods_names = [labels_dict[method] for method in methods]
    num_methods = len(methods)
    num_block_sizes = len(block_sizes)
    bar_width = 0.8 / num_block_sizes  # Total bar width per method is 0.8
    ...
    for i, b in enumerate(block_sizes):
        ...
        offset = (i - (num_block_sizes - 1) / 2) * bar_width
        positions = index + offset

        # Plot total counts (square + rectangular)
        bars_square = ax.bar(positions, list(square_counts[b].values()), 
                width=bar_width,
                label=methods_names,
                hatch=b_size_hatches[i],
                color=[color_dict[m] for m in methods],
                edgecolor='black')

        # Plot fumbles counts if applicable
        if fumbles:
            ax.bar(positions, list(worse_counts[b].values()), width=bar_width,
                color=fumbles_color,
                hatch = b_size_hatches[i],
                edgecolor='black')
    ...
    legend_elements.append(Patch(facecolor=fumbles_color, edgecolor='grey', label=f'Fumbles'))
...
def best_barplot_parameter(dfs_reordering,square_matrices, rectangular_matrices, method, plot_params_values = [], make_legend = True, plot_parameter = "mask",improvement_parameter = "None", ratio_parameter = "None", xlabel = "", ylabel = "", save_path = "", min_best = True):
...
def best_barplot_parameter_bsize(dfs_reordering, matrices, method, plot_params_values = [], block_sizes = [16,32], plot_parameter = "mask",improvement_parameter = "None", ratio_parameter = "None", xlabel = "", ylabel = "", save_path = "", min_best = True):
    ...
    bar_width = 0.8 / len(block_sizes)  # Total bar width per method is 0.8


    fig, ax = plt.subplots(figsize=(10, 5))

    for i,b in enumerate(block_sizes):
        improvement_parameter_b = f"{improvement_parameter}_{b}"
        ratio_parameter_b = f"{ratio_parameter}_{b}"

        index = np.arange(len(plot_params_values))
        offset = (i - (len(block_sizes) - 1) / 2) * bar_width
        positions = index + offset
        ...
        for plot_parameter_value in plot_params_values:
            dfs_parameter[plot_parameter_value] = df.loc[df[plot_parameter]==plot_parameter_value]
            dfs_parameter[plot_parameter_value] = find_best(dfs_parameter[plot_parameter_value], best_parameter=improvement_parameter_b, min=min_best)
            worse_counts[plot_parameter_value] = (dfs_parameter[plot_parameter_value][ratio_parameter_b] < .98).sum()

        matrix_counts = list(count_best_method(dfs_parameter,matrices,improvement_parameter_b).values())

        # Plot total counts (square + rectangular)
        bars_square = ax.bar(positions, matrix_counts, 
                width=bar_width,
                label=plot_params_labels,
                hatch=b_size_hatches[i],
                color=colors,
                edgecolor='black')

        # Plot fumbles counts if applicable
        ax.bar(positions, [-worse_counts[val] for val in plot_params_values], 
            width=bar_width,
            color=fumbles_color,
            hatch = b_size_hatches[i],
            edgecolor='black')
...
def make_improvements_barplot_and_distribution_2(dfs_reordering, methods, matrices, allow_missing=False, min_best=False, parameter="None", ylabel="Speedup", save_path="test.png", verbose=False):
    ...
    y_position = bottom - 0.08  # Adjust as needed to position the label below the histograms
    ...
    #ax_point.xaxis.set_label_coords(current_x_coord, current_y_coord - 0.06 )  # Adjust y-coordinate only
...
def plot_improvement_by_matrix(dfs_reordering, order_by, methods, parameter="None", matrices=None, min_best=False, title="", y_scale = "linear", original_line_y = 0, x_scale = "linear", yFormatter=lambda y, pos: y, ylim=[0, 5], fumble_area = [-10,1],xlabel="Default x Label", ylabel="Default Y Label", save_path="test_speedup_by_matrix.png"):
    """
    Plots the speedup for each matrix, sorted by speedups, for each method.
    ...
    # Top legend
    if "bsr" in parameter: #add failures to legend if BSR 
        plt.gca().yaxis.set_major_formatter(FuncFormatter(custom_yFormatter))
        plt.scatter([], [], color='gray', marker='x', label='Failures', s=35)
    else:
        plt.gca().yaxis.set_major_formatter(FuncFormatter(yFormatter))
    ...
    plt.subplots_adjust(top=1)  # Adjust the top margin as needed
    plt.axhspan(ymin=fumble_area[0], ymax=fumble_area[1], facecolor='#FFCCCC', alpha=0.2)
...
def plot_improvement_by_parameter_and_distribution(dfs_reorderings, plot_parameter, method = "clubs", improvement_parameter="None", allow_missing = True, matrices=None, min_best=False, title="", ylim=[0, 5], xlabel="Default x Label", ylabel="Default Y Label", save_path="test_parameter_study.png"):
    ...
    #ax_point.legend(loc='best', frameon=False)
    ax_point.set_ylabel(ylabel) 
    ax_point.set_xlabel(xlabel) 
    ax_point.set_ylim(min_y, max_y)
    ax_point.grid(True, linestyle='--', alpha=0.3)
    ax_point.tick_params(axis='both', which='major', length=5, direction='in', labelsize = 9)
    ...
    for i, plot_parameter_value in enumerate(plot_params_values):
        ...
        hist_axes[i].set_xlim(0, 80)  # Auto-adjust x-limits for the histogram
        hist_axes[i].set_ylim(ax_point.get_ylim())
        #hist_axes[i].xaxis.set_major_formatter(FuncFormatter(hist_formatter))
        hist_axes[i].set_yticklabels([])  # Hide the y-ticks on the histogram
        hist_axes[i].set_xticks([10, 30, 50, 70])  # Customize tick positions away from the edges
        hist_axes[i].grid(True, linestyle='--', alpha=0.3)
        hist_axes[i].tick_params(axis='x', labelsize = 9)  
    ...
    current_x_coord, current_y_coord = ax_point.xaxis.get_label().get_position()
    ax_point.xaxis.set_label_coords(current_x_coord, current_y_coord - 0.063 )  # Adjust y-coordinate only
